# gradcam_occlusion_smooth.py
"""
Occlusion-sensitivity saliency maps with smoothed overlays.
Saves overlays to gradcam_occlusion_outputs/ by default.

Usage:
    python gradcam_occlusion_smooth.py
    python gradcam_occlusion_smooth.py --debug_one "<path-to-image>" --sigma 2
"""
import os
import sys
import pathlib
...
OUT_DIR = os.path.join(ROOT, "gradcam_occlusion_outputs2")
IMG_SIZE = (224, 224)
...
def smooth_and_resize_heatmap(heatmap, out_size, sigma=2):
    """
    Apply gaussian blur (PIL) and resize with bicubic interpolation.
    heatmap: numpy HxW normalized 0..1
    out_size: (width, height) tuple (PIL format)
    sigma: gaussian blur radius (float). 0 => no blur.
    """
    # convert to PIL grayscale image
    h_img = Image.fromarray(np.uint8(heatmap * 255))
    if sigma > 0:
        h_img = h_img.filter(ImageFilter.GaussianBlur(radius=sigma))
    # PIL size expects (width, height)
    h_img = h_img.resize(out_size, resample=Image.BICUBIC)
    return np.array(h_img).astype(np.float32) / 255.0

def overlay_heatmap(original_pil, heatmap, outpath, alpha=ALPHA, sigma=2):
    """
    original_pil: PIL.Image (RGB)
    heatmap: low-res numpy HxW (0..1)
    sigma: gaussian blur radius applied BEFORE upsampling (higher => smoother)
    """
    sm = smooth_and_resize_heatmap(heatmap, original_pil.size, sigma=sigma)
    heatmap_img = Image.fromarray(np.uint8(cm.jet(sm) * 255))
    blended = Image.blend(original_pil.convert("RGB"), heatmap_img.convert("RGB"), alpha=alpha)
    blended.save(outpath)

def main(debug_one=None, sigma=2):
    ...
    for (img_path, true_label) in to_process:
        try:
            ...
            overlay_heatmap(pil_img.resize(target_size), heatmap, out_path, alpha=ALPHA, sigma=sigma)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--sigma", type=float, default=2.0, help="Gaussian blur radius for smoothing heatmap before upsampling")
    args = parser.parse_args()
    main(debug_one=args.debug_one, sigma=args.sigma)
